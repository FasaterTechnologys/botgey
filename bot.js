...
vk.api.messages.send({
	...
	vk.api.messages.send({ user_id: users[id].id, message: `
...
cmd.hear(/^(?:реф)\s([^]+)$/i, async (message, bot) => {
	...
	vk.api.messages.send({ user_id: users[ref[message.args[1]].id].id, message: `
...
cmd.hear(/^(?:топ|🏅Топ)$/i, async (message, bot) => {
	...
	vk.api.messages.send({peer_id: message.peerId, message: text, 
